...
class MotionGPT(BaseModel):
    ...
    def train_lm_forward(self, batch):
        ...
        inputs:
        bs = 2
        text: ['A person lying on his belly on the ground is standing up ending in a bit bent over stance.', 'A person is shuffling forward then taking a turn to the right.']
        tokens_ref.shape
            torch.Size([2, 25])

        task
        [{'class': 'predict', 'input': [...], 'output': [...]}, {'class': 'predict', 'input': [...], 'output': [...]}]
        ...
        print("texts", texts)
        print("tasks", tasks)
        print("tokens_ref", tokens_ref.shape)
        print("outputs", outputs)
        # outputs = self.t2m_gpt.gener
    ...
    def allsplit_step(self, split: str, batch, batch_idx):
        ...
        # Compute the metrics
        if split in ["val", "test"] and (self.current_epoch+1) % 100 == 0:
            print("running  val eveyr 100")
