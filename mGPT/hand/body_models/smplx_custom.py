...
from smplx.utils import (
    ...
    ModelOutput,
    FLAMEOutput,
    find_joint_kin_chain, dataclass)
from smplx.vertex_joint_selector import VertexJointSelector
from mGPT.hand.body_models.lbs_custom import lbs_custom
...
class SMPLH(SMPL):
    ...
    NUM_BODY_JOINTS = SMPL.NUM_JOINTS - 2 # 23 - 2 = 21
    NUM_HAND_JOINTS = 15
    NUM_JOINTS = NUM_BODY_JOINTS + 2 * NUM_HAND_JOINTS # 51
...
class SMPLX(SMPLH):
    ...
    NUM_HAND_JOINTS = 15
    NUM_FACE_JOINTS = 3
    NUM_JOINTS = NUM_BODY_JOINTS + 2 * NUM_HAND_JOINTS + NUM_FACE_JOINTS # 54
    ...
    def smplx_to_mano_params(
        ...
        **kwargs
        ...
        vertices, joints, joint_wise_global_pose = lbs_custom(shape_components, full_pose, self.v_template,
                               shapedirs, self.posedirs,
                               self.J_regressor, self.parents,
                               self.lbs_weights, pose2rot=pose2rot,
                               )
        ...
        smpl_body_joints = joints[:, :22] # N x 22x 2
        left_wrist_global_transl = smpl_body_joints[:, 20] # N x 127 x 3
        right_wrist_global_transl = smpl_body_joints[:, 21]

        
        smpl_joint_wise_global_pose = joint_wise_global_pose[:, :22]
        left_wrist_rot = smpl_joint_wise_global_pose[:, 20]
        right_wrist_rot = smpl_joint_wise_global_pose[:, 21]

        mano_hand_pose = {
            # left
            "left_global_transl": left_wrist_global_transl.cpu(),
            "left_global_pose_rot": left_wrist_rot.cpu(),
            "left_hand_pose_rot": left_hand_pose.cpu(),
            
            # right hand pose
            "right_global_transl": right_wrist_global_transl.cpu(),
            "right_global_pose_rot": right_wrist_rot.cpu(),
            "right_hand_pose_rot": right_hand_pose.cpu(),
        }

        return  mano_hand_pose
...
class SMPLXLayer(SMPLX):
    ...
    def smplx_to_mano_params(
        ...
        **kwargs
        ...
        smpl_body_joints = joints[:, :22] # N x 22 
        smpl_body_joints = smpl_body_joints
        left_wrist_global_transl = smpl_body_joints[:, 20] # N x 127 x 3
        right_wrist_global_transl = smpl_body_joints[:, 21]
...
class MANO(SMPL):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # transl = torch.tensor([0,0,0], dtype=dtype)
        # self.register_buffer('transl_flat', transl)


        ### adding the joint tips
        self.tip_joints = [745, 317, 445, 556, 673]
    ...
    def forward(
        self,
        betas: Optional[Tensor] = None, # B x 10
        global_orient: Optional[Tensor] = None, # B x 3
        hand_pose: Optional[Tensor] = None, # B x 45
        transl: Optional[Tensor] = None, # B x 3
        return_verts: bool = True,
        return_full_pose: bool = False,
        **kwargs
    ) -> MANOOutput:
        ...
        assert len(betas.shape) == 2 if betas is not None else True, f"Expected betas.shape[0] to be 2, but got {betas.shape if betas is not None else None}"

        assert len(hand_pose.shape) == 2 if hand_pose is not None else True, f"Expected hand_pose to be 2, but got {hand_pose.shape if betas is not None else None}"
        ...
        if joints is not None:
            tip_joints = vertices[:, self.tip_joints] # Bs x 5 x 3
        ...
        output = MANOOutput(vertices=vertices if return_verts else None,
                            ...
                            joints_w_tip=joints_w_tip if return_verts else None,
                            betas=betas,
                            global_orient=global_orient,
                            hand_pose=hand_pose,
                            full_pose=full_pose if return_full_pose else None)
    ...
    def batch_forward(
        ...
        **kwargs
        ...
        apply_trans = transl is not None or hasattr(self, 'transl_flat')
        if transl is None:
            if hasattr(self, 'transl_flat'):
                transl = self.transl_flat

        if self.use_pca:
            hand_pose = torch.einsum(
                'bi,ij->bj', [hand_pose, self.hand_components])

        full_pose = torch.cat([global_orient, hand_pose], dim=1) # Bs x J+1 x 3
        full_pose += self.pose_mean

        if betas.shape[0] != full_pose.shape[0]:
            betas = betas[0].expand(full_pose.shape[0],-1)
...
def create(
    ...
    **kwargs
    ...
    elif "smplxlayer" in model_type.lower():
        return SMPLXLayer(model_path, **kwargs)
    elif 'mano' in model_type.lower():
        return MANO(model_path, **kwargs)
    elif 'manolayer' in model_type.lower():
        return MANOLayer(model_path, **kwargs)
    elif 'flame' in model_type.lower():
