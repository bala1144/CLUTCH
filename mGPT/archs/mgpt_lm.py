...
class MLM(nn.Module):
    ...
    def placeholder_fulfill(self, prompt: str, length: int, motion_string: str,
        ...
        # predict_head = self.predict_ratio percetange, plus 1 for the start token
        masked_head = int(token_length * self.inbetween_ratio + 1)
        masked_tail = int(token_length * (1 - self.inbetween_ratio) + 1)
        
        motion_predict_head = '>'.join(motion_splited[:predict_head]) + f'><motion_id_{self.m_codebook_size+1}>'
        motion_predict_last = f'<motion_id_{self.m_codebook_size}>' + '>'.join(
            motion_splited[predict_head:])

        motion_masked = '>'.join(motion_splited[:masked_head]) + '>' + f'<motion_id_{self.m_codebook_size+2}>' * (masked_tail - masked_head) + '>'.join(motion_splited[masked_tail:])
        ...
        prompt = prompt.replace('<Motion_Placeholder_s1>', motion_predict_head) # used for inbetween and next token prediction during pretraining
        prompt = prompt.replace('<Motion_Placeholder_s2>',motion_predict_last) # used for inbetween and next token prediction during pretraining
        prompt = prompt.replace('<Motion_Placeholder_Masked>', motion_masked) # used for inpainting
