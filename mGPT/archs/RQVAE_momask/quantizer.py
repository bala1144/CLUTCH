...
class QuantizeEMAReset(nn.Module):
    def __init__(self, code_num, code_dim, mu=0.99):
    ...
    def reset_codebook(self):
        ...
        self.register_buffer('codebook', torch.zeros(self.code_num, self.code_dim, requires_grad=False))

    def _tile(self, x):
        code_num_x, code_dim = x.shape
        if code_num_x < self.code_num:
            n_repeats = (self.code_num + code_num_x - 1) // code_num_x
    ...
    def init_codebook(self, x):
        out = self._tile(x)
        self.codebook = out[:self.code_num]
        self.code_sum = self.codebook.clone()
        self.code_count = torch.ones(self.code_num, device=self.codebook.device)
        self.init = True
    ...
    def compute_perplexity(self, code_idx):
        ...
        code_onehot = torch.zeros(self.code_num, code_idx.shape[0], device=code_idx.device)  # code_num, N * L
        code_onehot.scatter_(0, code_idx.view(1, code_idx.shape[0]), 1)

        code_count = code_onehot.sum(dim=-1)  # code_num
    ...
    def update_codebook(self, x, code_idx):
        code_onehot = torch.zeros(self.code_num, x.shape[0], device=x.device) # code_num, N * L
        code_onehot.scatter_(0, code_idx.view(1, x.shape[0]), 1)

        code_sum = torch.matmul(code_onehot, x) # code_num, c
        code_count = code_onehot.sum(dim=-1) # code_num

        out = self._tile(x)
        code_rand = out[:self.code_num]

        # Update centres
        self.code_sum = self.mu * self.code_sum + (1. - self.mu) * code_sum
        self.code_count = self.mu * self.code_count + (1. - self.mu) * code_count

        usage = (self.code_count.view(self.code_num, 1) >= 1.0).float()
        code_update = self.code_sum.view(self.code_num, self.code_dim) / self.code_count.view(self.code_num, 1)
