...
# def create_action_rec_model(action_model, device="cpu"):

#     if action_model == "O2O":
#         model_cfg = "/path/to/checkpoints/ac_00_train/args.yml"
#         ckpt_path = "/path/to/checkpoints/ac_00_train/model.pt"
#     elif action_model== "S2O":
...
#     action_rec_model = instantiate_from_config(model_cfg)
#     action_rec_model.init_from_ckpt(ckpt_path)
#     action_rec_model.to(device)
#     action_rec_model.eval()
#     return action_rec_model


class Egovid5M_Metrics(Metric):
    def __init__(self,
                 cfg,
                 dataname='GRAB',
                 top_k=3,
        ...
        self.metrics = ['Recon', "JTS_Recon", "wrist_JT_recon"
    ...
    def compute(self, sanity_flag):
        ...
        all_gt_motions = torch.concatenate(self.gtmotion_feat, dim=0)
        all_pred_motions = torch.concatenate(self.recmotion_feat, dim=0)
 
        bs = all_gt_motions.shape[0]
        recon = torch.norm(all_pred_motions.reshape(bs, -1) - all_gt_motions.reshape(bs,-1), p=2, dim=-1).mean(0)
        metrics["Recon"] = recon

        ### compute all Joints recon metric
        JTS_pred = torch.concatenate(self.recmotion_joints, dim=0)
        JTS_gt =torch.concatenate(self.gtmotion_joints, dim=0)
        JTS_recon = torch.norm(JTS_pred.reshape(bs, -1) - JTS_gt.reshape(bs,-1), p=2, dim=-1).mean(0)
        metrics["JTS_Recon"] = JTS_recon

        ### compute all Wrist recon metric
        left_writst_joint = JTS_pred[:, :, 1, :]
        right_writst_joint = JTS_pred[:, :, 21, :]
        wrist_JT_pred = torch.cat([left_writst_joint, right_writst_joint], dim=-1) # N x T X 6
        assert wrist_JT_pred.shape[-1] == 6

        left_writst_joint = JTS_gt[:, :, 1, :]
        right_writst_joint = JTS_gt[:, :, 21, :]
        wrist_JT_gt= torch.cat([left_writst_joint, right_writst_joint], dim=-1) # N x T X 6
        assert wrist_JT_gt.shape[-1] == 6

        wrist_JT_recon = torch.norm(wrist_JT_pred.reshape(bs, -1) - wrist_JT_gt.reshape(bs,-1), p=2, dim=-1).mean(0)
        metrics["wrist_JT_recon"] = wrist_JT_recon
    ...
    def update(self,
        ...
        ### need to implement a new action classifier 
