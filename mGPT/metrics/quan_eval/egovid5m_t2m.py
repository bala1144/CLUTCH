...
class Egovid5M_TM2TMetrics(Metric):
    def __init__(self,
                 cfg,
                 dataname='humanml3d',
                 top_k=3,
        ...
        # MMDist
        self.add_state("gt_MMDist", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.metrics.append("gt_MMDist")
        self.add_state("MMDist", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.metrics.append("MMDist")
        ...
        # added by bala
        self.add_state("KID_mean", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("KID_std", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.metrics.append("KID_mean")
        self.metrics.append("KID_std")
    ...
    def _get_t2m_evaluator(self, cfg):
        ...
        # # init module
        # self.t2m_textencoder = instantiate_from_config(cfg.model.motion_vae.params.t2m_textencoder)
        # self.t2m_moveencoder = instantiate_from_config(cfg.model.motion_vae.params.t2m_moveencoder)
        # self.t2m_motionencoder = instantiate_from_config(cfg.model.motion_vae.params.t2m_motionencoder)
    ...
    def compute(self, sanity_flag):
        ...
        # assert len(self.text_embeddings) == len(self.recmotion_embeddings)
        # assert all(t.shape[0] == m.shape[0] for t, m in zip(self.text_embeddings, self.recmotion_embeddings))
        ...
        if self.text:
            ...
            assert all_texts.shape == all_gtmotions.shape
        ...
        if self.text:
            all_texts = all_texts.numpy()
            metrics["MMDist"] = compute_mmdist(all_texts, all_genmotions)
            metrics["gt_MMDist"] = compute_mmdist(all_texts, all_gtmotions)

        # Compute fid
        mu, cov = calculate_activation_statistics_np(expit(all_genmotions))
        gt_mu, gt_cov = calculate_activation_statistics_np(expit(all_gtmotions))
        # gt_mu, gt_cov = calculate_activation_statistics_np(all_gtmotions)
        metrics["FID"] = calculate_frechet_distance_np(gt_mu, gt_cov, mu, cov)

        # LID
        KID_met = calculate_kid(all_gtmotions, all_genmotions)
        metrics["KID_mean"] = KID_met[0]
        metrics["KID_std"] = KID_met[1]
    ...
    def update(self,
        ...
        feats_ref = feats_ref[align_idx] # [32, 60, 251]
        lengths_ref = np.array(lengths_ref)[align_idx] ## sorting by descending order

        gtmotion_embeddings = self.get_motion_embeddings(
            feats_ref, lengths_ref) # compute the get motion embeddings
        
        # gtmotion_embeddings.shape = [32, 512]
        ...
        # T2m text encoder
        if self.text:
            if word_embs.device != list(self.t2m_textencoder.parameters())[0].device:
                device = list(self.t2m_motionencoder.parameters())[0].device
                word_embs = word_embs.to(device)
                pos_ohot = pos_ohot.to(device)
            ...
            # assert text_embeddings.shape == gtmotion_embeddings.shape
        assert recmotion_embeddings.shape == gtmotion_embeddings.shape
