...
class Egovid5M_M2TMetrics(Metric):
    ...
    def update(self,
        ...
        #### only using the NLP metrics
        ## needs to trained, hence removing it for now
        # motion encoder
        ...
        if feats_ref.device != list(self.t2m_motionencoder.parameters())[0].device:
            feats_ref = feats_ref.to(list(self.t2m_motionencoder.parameters())[0].device)

        ref_mov = self.t2m_moveencoder(feats_ref).detach()
